...
def save_Yolo(img, boxes, save_path, prefix, image_name):
    ...
    if not os.path.exists(save_path) or not os.path.exists(
    ...
    try:
        ...
        with open(
            os.path.join(
                save_path,
                "labels",
                prefix + image_name[0 : len(image_name) - 4] + ".txt",
            ),
            "w",
            encoding="utf-8",
        ) as f:
...
def runAugumentation(image_path, label_path, save_path):
    ...
    for image_name in image_list:
        ...
        save_Yolo(
            to_image(t_img), boxes, save_path, prefix="rb_", image_name=image_name
        )
        # random_contrast 对比度变化
        t_img, t_boxes = DAD.random_contrast(img.clone()), boxes
        save_Yolo(
            to_image(t_img), boxes, save_path, prefix="rc_", image_name=image_name
        )
        # random_saturation 饱和度变化
        t_img, t_boxes = DAD.random_saturation(img.clone()), boxes
        save_Yolo(
            to_image(t_img), boxes, save_path, prefix="rs_", image_name=image_name
        )
        # 高斯噪声
        t_img, t_boxes = DAD.add_gasuss_noise(img.clone()), boxes
        save_Yolo(
            to_image(t_img), boxes, save_path, prefix="gn_", image_name=image_name
        )
        # add_salt_noise
        t_img, t_boxes = DAD.add_salt_noise(img.clone()), boxes
        save_Yolo(
            to_image(t_img), boxes, save_path, prefix="sn_", image_name=image_name
        )
        # add_pepper_noise
        t_img, t_boxes = DAD.add_pepper_noise(img.clone()), boxes
        save_Yolo(
            to_image(t_img), boxes, save_path, prefix="pn_", image_name=image_name
        )
...
if __name__ == "__main__":
    # 图像和标签文件夹
    image_path = r"E:\LLS\Target Detection\20230924clUG4uso\DataAugYolo-数据增强(1)\DataAugYolo-main\skin1\val\images"
    label_path = r"E:\LLS\Target Detection\20230924clUG4uso\DataAugYolo-数据增强(1)\DataAugYolo-main\skin1\val\labels"
    save_path = r"E:\LLS\Target Detection\20230924clUG4uso\DataAugYolo-数据增强(1)\DataAugYolo-main\skin1\val\Augumentation"  # 结果保存位置路径，可以是一个不存在的文件夹
    # 运行
